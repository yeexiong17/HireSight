...
// Add this new CandidateVideo component
    ...
    try {
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        const stream = await navigator.mediaDevices.getUserMedia({
          video: true,
          audio: isPreview ? false : true, // Audio false for preview, true for interview
        });
        if (videoRef.current) {
      ...
      alert(
        `Could not access webcam${
          isPreview ? " for preview" : " for the interview"
        }. Please ensure permissions are granted and no other app is using it.`
      );
    }
  };
          ...
          <CardContent className="flex-grow p-2">
            <div className="w-full h-full flex gap-4">
              ...
              <div className="w-3/4 bg-gray-900 rounded-md overflow-hidden">
                <CandidateVideo isPreview={false} className="" />
              </div>
            </div>
          </CardContent>
        </Card>
